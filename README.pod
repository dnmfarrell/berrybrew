...
B<berrybrew is up for adoption>. I no longer have a Windows machine, and am looking for someone to co-maintain or own the project. It requires minimal upkeep, just keeping up with new releases from L<http://strawberryperl.com>. There are also a bunch of interesting features that could be added if you have the appetite. Get in touch with me to discuss.
...
Start a new cmd.exe ...
    ...
    # run on only a selection of installed versions
...
Compile your own version with Mono:
    ...
    > git clone https://github.com/dnmfarrell/berrybrew
    > cd berrybrew
    > mcs -lib:lib -r:ICSharpCode.SharpZipLib.dll -r:Newtonsoft.Json.dll -out:bin/berrybrew.exe -win32icon:berrybrew.ico src/berrybrew.cs
    > cd bin
    > berrybrew.exe config
