...
home directory
- Added 'download' command (with Download() API method). Downloads one, or
all version(s) of Strawberry Perl portable
- Added `dev/generate_github_releases_json.pl` as a helper to keep the
perl version's `releases.json` file up-to-date
- Added "Update Releases JSON.md" documentation to help with the releases
update process
