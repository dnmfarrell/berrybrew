...
for my $file (bsd_glob("$dir/*.zip"), bsd_glob("$dir/*.msi")) {
    ...
    if ($file =~ m!/strawberry-perl(-ld|-no64)?-(5\.\d+\.\d+\.\d+)-(32bit|64bit)(-PDL|-portable)?\.(zip|msi)!) {
        ...
        my $relnotes = "https://strawberryperl.com/release-notes/$ver-$bits.html",
...
sub help {
    print qq{
        Usage: generate_github_releases_json.pl <options>

        Options:

        -d|--dir    Mandatory, string. The name of the directory to read perls from
        -t|--tag    Mandatory, string. The release tag that contains the perl files
        -h|--help   Optional, flag. Displays this help screen
    };
